...
class WP_Press_This {
	...
	private function _limit_img( $src ) {
		...
		if ( preg_match( '!/ad[sx]?/!i', $src ) ) {
			...
			// See mathtag.com and http://www.quantcast.com/how-we-do-it/iab-standard-measurement/how-we-collect-data/
			return '';
		...
		if ( ! empty( $data['_images'] ) ) {
			foreach ( $data['_images'] as $src ) {
				if ( false !== strpos( $src, 'gravatar.com' ) ) {
					$src = preg_replace( '%http://[\d]+\.gravatar\.com/%', 'http://secure.gravatar.com/', $src );
				}
