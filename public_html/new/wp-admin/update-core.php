	...
	if ( 'development' == $update->response ) {
		...
		if ( $current ) {
			...
			elseif ( !$mysql_compat )
				...
				$message = 	sprintf(__('You can update to <a href="http://codex.wordpress.org/Version_%1$s">WordPress %2$s</a> automatically:'), $update->current, $version_string);
			if ( !$mysql_compat || !$php_compat )
				$show_buttons = false;
		}
	}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="http://codex.wordpress.org/Dashboard_Updates_Screen">Documentation on Updating WordPress</a>' ) . '</p>' .
