...
<?php
	...
	static function fscf_wp_footer() {
        ...
        if ( isset(FSCF_Display::$add_recaptcha_script) && FSCF_Display::$add_recaptcha_script ) {
           //wp_enqueue_script( 'fscf-recaptcha', 'http://www.google.com/recaptcha/api.js' );
              // loads conditionally if forms have recaptcha enabled
              // makes multiforms compatible on same page
            wp_enqueue_script( 'fscf-recaptcha', 'http://www.google.com/recaptcha/api.js?onload=fscfReCAPTCHA&render=explicit' );
...
';
	...
	static function get_captcha_url_cf() {
	  ...
	  if ( is_ssl() ) {
			$captcha_url_cf = preg_replace('|http://|', 'http://', $captcha_url_cf);
	  }
