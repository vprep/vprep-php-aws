	...
	if ( $posts->have_posts() ) {
		...
		while ( $posts->have_posts() ) {
			...
			if ( date( 'Y-m-d', $time ) == $today ) {
				...
				/* translators: date and time format for recent posts on the dashboard, from a different calendar year, see http://secure.php.net/date */
				$relative = date_i18n( __( 'M jS Y' ), $time );
			} else {
				/* translators: date and time format for recent posts on the dashboard, see http://secure.php.net/date */
