...
class Educational_model extends CI_Model {
    ...
    */
	...
	//		$cb = $t['si_institute_id'];
			$arr = $this->db->query("SELECT * FROM `exam_list` WHERE `start_date` <= date('$dt') and `end_date` >= date('$dt') and exam_status=1 and `created_by` = 4")->result_array();
...
////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$createdBy=1;
         ...
         if($this->input->post('question_title'))
			$this->db->query("INSERT INTO study_material SET Question_title='".$this->input->post('question_title')."', subject_id='".$this->input->post('subject_id')."', module_id='".$this->input->post('module')."', sub_module_id='".$this->input->post('sub_module')."', question_type='".$this->input->post('question_type')."', Ques_type='".$this->input->post('Ques_type')."', ques_img='".$this->input->post('ques_img')."', difficulty_level='".$this->input->post('difficulty_level')."', tags='".$this->input->post('module_for_tag')."', title='".$this->input->post('title')."',Rating='4.".$random_star."',views='".$random_view."',solution='".$this->input->post('solution')."',created_by='3',updated_by='3', updated_at='', created_at='' ");
	...
	function save_test_questions($userid,$questionImage,$option1_img,$option2_img,$option3_img,$option4_img)
	{
 ...
 // `qiz_created_by`='$userId'")->result_array();
    ...
    function get_test_exam_questions($exam_id,$page=0)
	{
      ...
      //  $offset = ($page != 0) ? ((int) ($page-1) * 1) : 0;
      //  $this->db->limit(1,$page);
        return $this->db->get_where('vprep_exam_question as eq',array('eq.qiz_exam_id'=>$exam_id))->result_array();
    }
