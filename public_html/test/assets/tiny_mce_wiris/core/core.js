...
function wrs_getContent(url, postVariables) {
    try {
        ...
        if (httpRequest) {
            ...
            else if (url.substr(0, 1) == '/' || url.substr(0, 7) == 'http://' || url.substr(0, 8) == 'http://') {
...
function wrs_mathmlToImgObject(creator, mathml, wirisProperties, language) {
    ...
    // Role math http://www.w3.org/TR/wai-aria/roles#math.
...
function wrs_b64ToByteArray(b64String, len) {
    ...
    for (var i = 0; i < l; i += 4) {
        ...
        // See http://tracker.moodle.org/browse/CONTRIB-5862 for further information.
