    ...
    <div class="col-xs-12">
        <h5 style="padding:0px; margin:0px;">  
			...
			<?php if($test_question['ques_img']!=""){ ?>
				 <div><img src="<?php echo base_url(); ?>/uploads/<?php echo $test_question['ques_img']; ?>" style="height:auto;width:auto;"></div>
			<?php }?>
   ...
   $getRow = $test_exam_questions;
	...
	if($total_exam_question>0)
	{
		for($kp = 0; $kp < $total_exam_question; $kp++){
			...
			if(count($answered) > 0 && array_key_exists($getRow[$kp]['id'],$answered))
			{
				$color =  "btn btn-success";
				$style =  '';
			}
			else if(count($getFlag) > 0 && @in_array($getRow[$kp]['id'], $getFlag))
			{
				$color =  "btn btn-info";
				$style =  '';
			}
			else if(count($getAnsData) > 0 && @in_array($getRow[$kp]['id'], $getAnsData))
			{
				$color =  "btn btn-default";
				$style =  'style="background:#fff; border:1px solid #ccc; color:#737373;"';
			}
			else if(count($answered) > 0 && in_array($getRow[$kp]['id'], $answered) && @$answered[$getRow[$kp]['id']] == "" && @in_array($getRow[$kp]['id'], $getAnsData))
			{
