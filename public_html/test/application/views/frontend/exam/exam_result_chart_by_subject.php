...
foreach($total_ques_data as $row)
{
	if($row['correct_ans']==$sub_ans_total[$row['id']])
	{
		...
		@$sub_correct_answer[$row['subject_id']]++;
	}
	
	if(in_array($row['id'],$attept_ques_id))
	{
		@$total_attept_ques[$row['subject_id']]++;
	}
	 
	

	@$sub_ques_total[$row['subject_id']]++;
...
window.onload = function () {
	var chart = new CanvasJS.Chart("chartContainer1",
	{
		...
		data: [
		{
			...
			$subInfo = $this->stu->getSubjectDetail($subkey);
			?>
			{  y: 53.37, legendText:"<?php echo $subInfo->subject_title; ?> <?php  echo (($total_attept_ques[$subkey]*100)/$subVal) ?>%", indexLabel: "<?php echo $subInfo->subject_title; ?> <?php  echo (($total_attept_ques[$subkey]*100)/$subVal) ?>%" },
			<?php } ?>
			]
		}
		]
...
var chart = new CanvasJS.Chart("chartContainer2",
	{
		...
		{        
			...
			$subInfo = $this->stu->getSubjectDetail($subkey);
			?>
				{  y: <?php  echo (($sub_correct_answer[$subkey]*100)/$subVal) ?>, legendText:"<?php echo $subInfo->subject_title; ?>", indexLabel: "<?php echo $subInfo->subject_title; ?>" },
			<?php
				}
			?>
			]
