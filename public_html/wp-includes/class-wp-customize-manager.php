...
final class WP_Customize_Manager {
		...
		if ( ! empty( $attachments ) ) {
			...
			foreach ( $attachments as $symbol => $attachment ) {
				...
				if ( ! $attachment_id ) {
					...
					// See http://bugs.php.net/bug.php?id=65701
	...
	public function remove_panel( $id ) {
		...
		if ( in_array( $id, $this->components, true ) ) {
			...
			$message = sprintf( __( 'Removing %1$s manually will cause PHP warnings. Use the %2$s filter instead.' ),
				$id,
				'<a href="' . esc_url( 'http://developer.wordpress.org/reference/hooks/customize_loaded_components/' ) . '"><code>customize_loaded_components</code></a>'
			);
	...
	public function register_controls() {
		...
		if ( current_theme_supports( 'custom-header', 'video' ) ) {
			...
			// @todo Customizer sections should support having notifications just like controls do. See <http://core.trac.wordpress.org/ticket/38794>.
		...
		$this->add_control( new WP_Customize_Media_Control( $this, 'header_video', array(
			...
			'mime_type'      => 'video',
			// @todo These button_labels can be removed once WP_Customize_Media_Control provides mime_type-specific labels automatically. See <http://core.trac.wordpress.org/ticket/38796>.
			'button_labels'  => array(
		...
		/*
		 * Static Front Page
		 * See also http://core.trac.wordpress.org/ticket/19627 which introduces the the static-front-page theme_support.
		...
		$this->add_section( 'custom_css', array(
			...
			'description'        => sprintf( '%s<br /><a href="%s" class="external-link" target="_blank">%s<span class="screen-reader-text">%s</span></a>',
				...
				esc_url( __( 'http://codex.wordpress.org/CSS' ) ),
