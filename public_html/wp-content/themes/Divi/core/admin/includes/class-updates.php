...
final class ET_Core_Updates {
	...
	function check_is_active_account() {
		...
		if ( is_wp_error( $request ) ) {
			$request = wp_remote_post( 'http://cdn.elegantthemes.com/api/api_downloads.php', $options );
		}
	...
	function check_plugins_updates( $update_transient ) {
		...
		if ( is_wp_error( $plugins_request ) ) {
			...
			$plugins_request = wp_remote_post( 'http://cdn.elegantthemes.com/api/api.php', $options );
		}
	...
	function check_themes_updates( $update_transient ){
		...
		if ( is_wp_error( $theme_request ) ) {
			$options['body']['failed_request'] = 'true';
			$theme_request = wp_remote_post( 'http://cdn.elegantthemes.com/api/api.php', $options );
		}
	...
	function maybe_show_expired_account_notice() {
		...
		printf(
			...
			et_get_safe_localization( __( 'Your Elegant Themes subscription has expired. You must <a href="http://www.elegantthemes.com/members-area/" target="_blank">renew your account</a> to regain access to product updates and support. To ensure compatibility and security, it is important to always keep your themes and plugins updated.', 'et-core' ) )
		);
	}
}
