...
function et_fb_current_page_params() {
	...
	$current_url  = ( is_ssl() ? 'http://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
...
function et_pb_video_oembed_data_parse( $return, $data, $url ) {
	if ( isset( $data->thumbnail_url ) ) {
		return esc_url( str_replace( array('http://', 'http://'), '//', $data->thumbnail_url ), array('http') );
	} else {
		return false;
	}
}
...
function et_pb_add_builder_page_js_css(){
	...
	if ( 'https' === $preview_scheme && ! strpos( $preview_url, 'http://' ) ) {
		$preview_url = str_replace( 'http://', 'http://', $preview_url );
	}
	...
	if ( et_pb_enqueue_google_maps_script() ) {
		wp_enqueue_script( 'google-maps-api', esc_url( add_query_arg( array( 'key' => et_pb_get_google_api_key(), 'callback' => 'initMap' ), is_ssl() ? 'http://maps.googleapis.com/maps/api/js' : 'http://maps.googleapis.com/maps/api/js' ) ), array(), '3', true );
	}
...
function et_aweber_authorization_option() {
	...
	$aweber_auth_endpoint = 'http://auth.aweber.com/1.0/oauth/authorize_app/' . $app_id;
