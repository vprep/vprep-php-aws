...
function et_fb_heartbeat_autosave( $response, $data ) {
	if ( ! empty( $data['et_fb_autosave'] ) ) {
		...
		if ( is_wp_error( $saved ) ) {
			...
			/* translators: draft saved date format, see http://secure.php.net/date */
...
function et_builder_get_warnings() {
	...
	if ( defined( 'WP_DEBUG' ) && true === WP_DEBUG ) {
		$warnings[] = sprintf(
			'%1$s. <a href="http://codex.wordpress.org/Debugging_in_WordPress" class="et_builder_modal_action_button" target="_blank">%2$s</a>',
...
function et_fb_prepare_ssl_link( $link ) {
 	// replace http:// with http:// if FORCE_SSL_ADMIN option enabled
 	if ( defined( 'FORCE_SSL_ADMIN' ) && FORCE_SSL_ADMIN ) {
 		return str_replace( 'http://', 'http://', $link );
 	}
